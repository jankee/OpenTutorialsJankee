...
public class NsEffectManager : MonoBehaviour
{
	...
	public static void RunReplayEffect(GameObject instanceObj, bool bClearOldParticle)
	{
        ...
        //feeltest
        NcEffectAniBehaviour[] ncAniComs = instanceObj.GetComponentsInChildren<NcEffectAniBehaviour>(true);
        foreach (NcEffectAniBehaviour ncAniCom in ncAniComs)
        {
            ncAniCom.OnResetReplayStage(bClearOldParticle);
        }
        //~feeltest
	}
	...
	public static void AdjustSpeedEditor(GameObject target, float fSpeedRate)	// support shuriken, legancy, mesh
	{
        ...
        //feeltest
        NcEffectAniBehaviour[] ncAniComs = target.GetComponentsInChildren<NcEffectAniBehaviour>(true);
        foreach (NcEffectAniBehaviour ncAniCom in ncAniComs)
        {
            ncAniCom.OnUpdateEffectSpeed(fSpeedRate, false);
        }
        //~feeltest
	}
#endif
	public static void AdjustSpeedRuntime(GameObject target, float fSpeedRate)	// support legancy/mesh , not support shuriken
	{
        ...
        foreach (NcEffectAniBehaviour ncAniCom in ncAniComs)
        {
            ncAniCom.OnUpdateEffectSpeed(fSpeedRate, true);
        }
        //~feeltest
	}

    //feeltest
    public static void SetTimeScale(GameObject target, float fTimeScale = 1f)
    {
        NcEffectAniBehaviour[] ncAniComs = target.GetComponentsInChildren<NcEffectAniBehaviour>(true);
        foreach (NcEffectAniBehaviour ncAniCom in ncAniComs)
        {
            ncAniCom.SetTimeScale(fTimeScale);
        }
    }
    //~feeltest
